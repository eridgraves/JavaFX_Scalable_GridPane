...
public class PaneDemo extends Application {
    ...
    static Shape createShape() { // TODO: somehow tie this to the pane's position/size?
    ...
    public void start(Stage primaryStage) {
        ...
        double col_size = 100 / WORLD_WIDTH;
        double row_size = 100 / WORLD_HEIGHT; // as percentages
        System.out.println(WORLD_WIDTH + " COLS: " + col_size + "\n" + WORLD_HEIGHT + " ROWS: " + row_size);

        ColumnConstraints c_con = new ColumnConstraints();
        for (int col = 0; col < WORLD_WIDTH; col++) {
            c_con.setPercentWidth(col_size);
            grid.getColumnConstraints().add(c_con);
        }

        RowConstraints r_con = new RowConstraints();
        for (int r = 0; r < WORLD_HEIGHT; r++) {
            r_con.setPercentHeight(row_size);
            grid.getRowConstraints().add(r_con);
        }

        for(int r = 0; r < WORLD_HEIGHT; r++){
            for(int c = 0 ; c< WORLD_WIDTH; c++){
                Region square = createDefault();
                //TODO: Check to see if critters should be added to the square -- Grouping the Critter shape to the square region should allow them to scale together
                if(c%2 == 1) {
                    Rectangle rect1 = new Rectangle(10, 10, 200, 200);
                    rect1.setFill(Color.BLUE);
                    square.setBackground(new Background(new BackgroundFill(Color.BLUE, CornerRadii.EMPTY, Insets.EMPTY)));
                }


                square.setMinSize(10,10); // TODO: add min size?

                grid.setColumnIndex(square, c);
                grid.setRowIndex(square, r);
                grid.getChildren().add(square);
            }
        }
        btnDefault.setOnAction(t -> grid.getChildren().add(createDefault()));
        //btnClipped.setOnAction(t -> grid.getChildren().add(createClipped()));
        //btnNested.setOnAction(t -> grid.getChildren().add(createNested()));

        final Scene scene = new Scene(new HBox(buttons, grid));
